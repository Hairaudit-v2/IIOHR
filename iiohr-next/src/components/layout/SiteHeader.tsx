    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/98">
      <div className="relative mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3.5 sm:px-5">
        ...
        >
          <span
            aria-hidden="true"
            className="inline-flex h-9 w-9 items-center justify-center rounded-[6px] border border-heading/20 bg-surface text-[10px] font-semibold tracking-[0.15em] text-heading"
          >
            II
          </span>
          <span className="flex flex-col">
            <span className="text-lg font-semibold tracking-[0.05em]">IIOHR</span>
            <span className="mt-1 hidden text-[10px] tracking-[0.12em] text-readable-muted uppercase group-hover:text-foreground md:block">
        ...
        <nav className="hidden items-center gap-4.5 xl:flex">
          {primaryNavigation.map((item) => (
            item.external ? (
              ...
              <Link
                ...
                className={`px-1 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-intel/45 focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                  pathname === item.href
          ...
          <Button href="/apply" variant="secondary" className="ml-1.5">
            Apply for Training
          </Button>
        </nav>
        <MobileNav />

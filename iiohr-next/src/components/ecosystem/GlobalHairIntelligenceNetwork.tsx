...
import { useEffect, useMemo, useId, useSyncExternalStore, useState } from "react";
...
function getThemeColors(resolvedTheme: "light" | "dark") {
  ...
  return isDark
    ...
    : {
        ...
        ringStroke: "#C4C2BC",
        lineStroke: "#A8A59E",
        lineAnimated: "rgba(166,139,75,0.7)",
        centerFill: "url(#ghin-gold-center)",
        centerStroke: "#967A3D",
        centerGlow: "rgba(198,167,94,0.3)",
        outerFill: "#FFFFFF",
        outerStroke: "#B8B5AE",
        outerHighlightStroke: "rgba(150,122,61,0.65)",
        textPrimary: "#1C1A18",
        textSecondary: "#3C3A36",
        textMuted: "#5C5A56",
        networkLabel: "#5C5A56",
      };
}
...
function useResolvedTheme(theme: GlobalNetworkTheme): "light" | "dark" {
  ...
  return theme !== "auto" ? theme : systemTheme;
}
...
export function GlobalHairIntelligenceNetwork({
  ...
  const defsId = `ghin-${useId().replace(/:/g, "-")}`;

  return (
    <div
      className={`relative w-full max-w-[1000px] mx-auto min-h-[400px] ${className}`}
      style={{ aspectRatio: "1" }}
      role="img"
      aria-label={title}
    >
